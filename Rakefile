...
def gemspec
  @gemspec ||= begin
    file = File.expand_path('../c2dm_batch.gemspec', __FILE__)
...
task :gem do
  ...
  pkg = "#{root}/pkg"
  system "rm -Rf #{pkg}"
  system "cd #{root} && gem build c2dm_batch.gemspec"
...
task :rename do
  ...
  begin
    ...
    if from
      to = from.split('/')
      to[-1].gsub!('c2dm_batch', name)
      FileUtils.mv(from, to.join('/'))
    end
  end while dir.length > 0
  Dir["**/*"].each do |path|
    if File.file?(path)
      `sed -i '' 's/c2dm_batch/#{name}/g' #{path}`
      `sed -i '' 's/C2dmBatch/#{camelize.call(name)}/g' #{path}`
